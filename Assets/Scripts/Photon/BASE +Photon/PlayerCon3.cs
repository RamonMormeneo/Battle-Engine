...
public class PlayerCon3 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PV.IsMine)
        {
            ...
            myAvatar = PhotonNetwork.Instantiate(Path.Combine("PhotonPrefabs", "BaseMediana(Online)"), GameControler.GS.spawnpoint[SpawnPicker].transform.position, GameControler.GS.spawnpoint[SpawnPicker].transform.rotation, 0);
